...
class DataModelSep():
    ...
    def get_targets_dic(self, start=None, end=None):
        ...
        return {
            'typ_e_cluster': self._target_e_cluster[start:end],
            'typ_p_cluster': self._target_p_cluster[start:end],
            'pos_e_cluster': self._target_e_cluster[start:end],
            'pos_p_cluster': self._target_p_cluster[start:end],
            'enrg_e_cluster': self._target_e_cluster[start:end],
            'enrg_p_cluster': self._target_p_cluster[start:end],
            'typ_type': self._target_type[start:end],
            'pos_x': self._target_pos_x[start:end],
            'pos_y': self._target_pos_y[start:end],
            'pos_z': self._target_pos_z[start:end],
            'enrg_energy': self._target_energy[start:end]
        }
        ...
        while True:
            ...
            for step in range(self.steps_per_epoch):
                ...
                if augment:
                    ...
                    targets_batch['typ_e_cluster'][:,1] = np.where(np.equal(targets_batch['typ_e_cluster'][:,[1]], sequence))[1]
                    targets_batch['typ_p_cluster'][:,1] = np.where(np.equal(targets_batch['typ_p_cluster'][:,[1]], sequence))[1]
                    targets_batch['pos_e_cluster'][:,1] = np.where(np.equal(targets_batch['pos_e_cluster'][:,[1]], sequence))[1]
                    targets_batch['pos_p_cluster'][:,1] = np.where(np.equal(targets_batch['pos_p_cluster'][:,[1]], sequence))[1]
                    targets_batch['enrg_e_cluster'][:,1] = np.where(np.equal(
                            targets_batch['enrg_e_cluster'][:,[1]], sequence))[1]
                    targets_batch['enrg_p_cluster'][:,1] = np.where(np.equal(
                            targets_batch['enrg_p_cluster'][:,[1]], sequence))[1]
                    
                yield (
                    features_batch, 
                    targets_batch, 
                    targets_batch['typ_type'] * self.weight_compton + \
                        (1-targets_batch['typ_type']) * self.weight_non_compton
                )
